...
void Pelicula::copiar(Pelicula* original){
	titulo = original->obtener_titulo();
	genero = original->obtener_genero();
	puntaje = original->obtener_puntaje();
	director = original->obtener_director();

	Lista<std::string>* nueva_lista_actores = new Lista<std::string>;
	lista_actores = nueva_lista_actores;
	lista_actores->copiar(lista_actores, original->obtener_lista_actores() );
}
